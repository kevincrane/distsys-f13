    ...
    public List<Centroid> calculateAllCentroidsParallel(List<DataPoint> points, int numClusters) {
        ...
        try {
            ...
            for (int i = 0; i < numClusters; i++) {
                MPI.COMM_WORLD.Allreduce(totalA[i], 0, totalA[i], 0, strand.length, MPI.INT, MPI.SUM);
                MPI.COMM_WORLD.Allreduce(totalG[i], 0, totalG[i], 0, strand.length, MPI.INT, MPI.SUM);
                MPI.COMM_WORLD.Allreduce(totalC[i], 0, totalC[i], 0, strand.length, MPI.INT, MPI.SUM);
                MPI.COMM_WORLD.Allreduce(totalT[i], 0, totalT[i], 0, strand.length, MPI.INT, MPI.SUM);
            }
